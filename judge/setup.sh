...
if command -v uv >/dev/null 2>&1; then uv venv .venv --python 3.13 >/dev/null || exit 1; uv pip install --python .venv/bin/python pip -r requirements.lock || exit 1; else python3 -m venv .venv || exit 1; .venv/bin/pip install -r requirements.lock || exit 1; fi
echo "== Whisper models =="
mkdir -p models
