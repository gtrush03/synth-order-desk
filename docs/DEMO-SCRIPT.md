...
See the [full walkthrough](FULL-WALKTHROUGH.md#follow-the-demonstration) for the live sequence and the [README](../README.md#one-request-real-follow-through) for film chapter times. Preconditions: server built and started; HydraDB and Cognee running; company memory reset in both the local file and the Cognee dataset; Hotdata allowance either enabled with runs remaining or absent (local-file mode is labelled in the receipts).
